...
def plot_correlation(feature_df):
    '''
...
def compute_vif(X_const):
    ...
    vif_df = (
        pd.Series(vif_values, index=X_const.columns)
        .rename("VIF")
        .to_frame()
        .round(2)
    )
...
def run_full_analysis(filepath):
    ...
    return {
        "df": df, "y": y,
        "df_insens": df_insens, "df_sens": df_sens,  
        "results_insens": results_insens, "results_sens": results_sens,
        "vif_insens": vif_insens, "vif_sens": vif_sens,
        "insens_metrics": insens_metrics, "sens_metrics": sens_metrics,  
        "comparison": comparison,
        "test_pred_insens": test_pred_insens, "y_test_insens": y_test_insens,  
        "test_pred_sens": test_pred_sens, "y_test_sens": y_test_sens,          
    }
