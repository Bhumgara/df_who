...
if "model" in st.session_state.keys():
    ...
    if st.button(label="Predict life expectancy"):
        ...
        new_data.extend([schooling, developed, region_as, region_ca, region_eu, region_me, region_na, region_oc, region_re, region_sa])
        
        life_pred = make_prediction(st.session_state["data"], st.session_state["model"], new_data, exclude_sensitive=st.session_state["exclude_sensitive"])
